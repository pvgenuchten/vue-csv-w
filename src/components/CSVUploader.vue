...
<thead class="table-dark"><tr><th v-for="h in headers" :key="h">{{ h }}</th></tr></thead>
<tbody><tr v-for="(row,i) in rows.slice(0,5)" :key="i"><td v-for="h in headers" :key="h">{{ row[h] }}</td></tr></tbody>
</table>
...
export default {
emits: ['csv-loaded'],
data(){ return { rows: [], headers: [], csvText: '' } },
methods:{
onFileChange(e){
